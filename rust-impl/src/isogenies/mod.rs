...
fn eval_iso<PtT>(pt: PtT, coeffs: [&[CoordT<PtT>]; 4]) -> (CoordT<PtT>, CoordT<PtT>, CoordT<PtT>)
where
    PtT: CurveProjective,
{
    // unpack input point
    ...
    let zpows = {
        // XXX: In array below, 15 is long enough for both iso11 and iso3.
        // Would be nice to associate a constant with PtT telling us how big
        // to make this, but it turns out that's broken as of Rust 1.35.
    ...
    // XXX: In array below, 16 is long enough for both iso11 and iso3.
...
/// Evaluate 11-isogeny from E' to E
pub fn iso11(pt: G1) -> G1 {
    use self::g1_consts::{XDEN, XNUM, YDEN, YNUM};
    use pairing::bls12_381::transmute::g1_projective;

    let (x, y, z) = eval_iso(pt, [&XNUM[..], &XDEN[..], &YNUM[..], &YDEN[..]]);
    unsafe { g1_projective(x, y, z) }
}

/// Evaluate 3-isogeny from E2' to E2
pub fn iso3(pt: G2) -> G2 {
    use self::g2_consts::{XDEN, XNUM, YDEN, YNUM};
    use pairing::bls12_381::transmute::g2_projective;

    let (x, y, z) = eval_iso(pt, [&XNUM[..], &XDEN[..], &YNUM[..], &YDEN[..]]);
    unsafe { g2_projective(x, y, z) }
}
